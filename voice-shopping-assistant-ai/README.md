...
**No build step. No npm. No frameworks.** Just plain HTML, CSS, and JavaScript
(ES modules). Voice recognition and speech use the browser's built-in Web Speech
API, so the whole app is static files you can host anywhere for free. An optional
AI mode adds free-form understanding on top.
...
- **Conversational voice agent** — a reactive orb shows four states (idle,
  listening, thinking, speaking), and a live feed logs the back-and-forth.
- **Spoken replies** — Voxa reads confirmations aloud via speech synthesis.
- **Hands-free mode** — keep the mic open for back-to-back commands.
- **AI understanding (optional)** — with a free Google Gemini key, Voxa handles
  free-form and even recipe-style requests (*"I'm making tacos, add what I
  need"*). Without a key, it uses a fast built-in parser — so it always works.
...
Google Gemini (optional) · localStorage. No dependencies, no build tooling.
